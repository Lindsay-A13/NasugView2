...
if($search !== ''){
    $searchLike = '%' . $search . '%';
    $stmt->bind_param("isss", $owner_id, $searchLike, $searchLike, $searchLike);
} else {
...
<form class="order-search" method="GET" action="order_list.php">
    ...
    <?php if($search !== ''): ?>
    <a class="clear-search" href="order_list.php?tab=<?= urlencode($currentTab) ?>">Clear</a>
    <?php endif; ?>
</form>
</div>
...
<a class="tab <?= $currentTab === $tab ? 'active' : '' ?>" href="order_list.php?<?= htmlspecialchars(http_build_query(['tab' => $tab, 'search' => $search])) ?>">
