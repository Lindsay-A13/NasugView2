...
$stmt = $conn->prepare("
    SELECT
        ...
        b.business_name
    FROM services s
    INNER JOIN business_owner b ON s.owner_id = b.b_id
...
<title>All Services | NasugView</title>
...
<div class="page">
    ...
    <div class="grid">
        ...
        <a href="servicedetails.php?id=<?= (int) $row['id'] ?>" class="card">
            <img src="uploads/services/<?= htmlspecialchars($row['image'] ?: 'default_service.jpg') ?>" alt="<?= htmlspecialchars($row['name']) ?>">
            <div class="body">
                <div class="name"><?= htmlspecialchars($row['name']) ?></div>
                <div class="meta"><?= (int) $row['duration'] ?> hour<?= (int) $row['duration'] === 1 ? '' : 's' ?> • <?= htmlspecialchars($row['business_name']) ?></div>
